    ...
    def pairing(self, Animal):
        if self.name == Animal.name and self.sex != Animal.sex and self.age > 4:
            ...
            if n == 1:
                ...
                sex = "woman"
            print("Львица родила:")
            return Lion(self.name, sex)

    # повзрослеть
    def skip_year(self, year = 1):
        self.age += year
        if self.sex == "man":
            print(f"Лев {self.alias} повзрослел на {year} л.")
        else:
            print(f"Львица {self.alias} позврослела на {year} л.")

    # рявкнуть
    def squirm(self, animal):
        if animal.kind == "herbivores":
            if self.sex == "man":
                print(f"Лев {self.alias} зарычал, {animal.name} дал драпу")
            else:
                print(f"Львица {self.alias} зарычала, {animal.name} дал драпу")